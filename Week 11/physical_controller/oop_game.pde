...
Serial myPort;
int inByte;                      //incoming serial byte: 0--up 1--left 2--down 3--right 4--start/pause/resume
...
int wolf_num;                    //number of wolves (between 15 and 20)
...
class Bucket {
  ...
  void update() {
    if (inByte == 1) {             //left
      bucket_xpos --;
    } else if (inByte == 3) {      //right
      bucket_xpos ++;
    } else if (inByte == 0) {      //up
      bucket_ypos --;
    } else if (inByte == 2) {      //down
      bucket_ypos ++;
    }
...
void initialize() {
  ...
  wolf_num = (int)random(15, 21);       //randomize the total number of wolves (15-20) that will show up in the next round
  for (int i = 0; i < wolf_num; i++) {
    wolves[i] = new Wolf();
  }
}
...
void serialEvent(Serial myPort) {
  ...
  if (inByte == 4) {                               //start/pause/resume
    if (!start) {
      myPort.write(0);                             //start melody
    }
    start = true;
    pause = !pause;
  } else if (start && !pause && countdown < -1) {  //move position
    myBucket.update();
    myBucket.check_pos();
  }
}
